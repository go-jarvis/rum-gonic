...
import (
	...
	"github.com/go-jarvis/rum-gonic/pkg/middlewares/trace/idgen"
...
var (
	genor idgen.IDGenerator
)

func init() {
	genor = idgen.NewDefaultGenerator()
}
...
func newSpan(span trace.Span) trace.Span {
	...
	if spanCtx.TraceID().IsValid() {
		sid := genor.NewSpanID(ctx, span.SpanContext().TraceID())
		spanCtx = spanCtx.WithSpanID(sid)
		ctx = trace.ContextWithRemoteSpanContext(ctx, spanCtx)
	} else {
		tid, sid := genor.NewIDs(ctx)
		spanCtx = spanCtx.WithSpanID(sid)
